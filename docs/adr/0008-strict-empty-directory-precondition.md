# forge init requires a strictly empty directory

**Status:** accepted · 2026-06-19 (stated in the init-lifecycle spec §1.1; recorded as its own ADR 2026-06-20)

## Context

`forge init` is an ordered mutation that creates a whole project tree in the current
...
`forge init` **refuses to run unless the target directory is empty**, ignoring only inert
...
- The entire resulting tree is forge's own creation, so the blast radius of the ADR-0004
