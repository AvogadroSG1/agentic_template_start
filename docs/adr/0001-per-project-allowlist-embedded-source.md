...
**embedded in the `forge` binary** (`embed.FS`, like the golden templates). A reconciler
(`forge sync-allowlist`) rewrites the managed block from that source, triggered
...
- Refresh requires the `forge` binary present on the machine doing the refresh.
