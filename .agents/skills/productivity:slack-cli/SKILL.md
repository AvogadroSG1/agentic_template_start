...
Cache warming is explicit — run `slack-cli cache warm` to populate. Commands that use the cache will warn on stderr if it is stale but never block. Four separate files for fast lookups:
...
Both commands resolve user IDs to display names automatically using the local cache (`~/.slack-cli/id-to-name.json`). Run `slack-cli cache warm` to populate. If the cache is stale or empty, a warning is shown on stderr but the command proceeds. If a user ID is not in the cache, the raw ID is shown as a fallback.
...
| "Warning: cache not warmed" on stderr | Run `slack-cli cache warm` to populate the cache |
